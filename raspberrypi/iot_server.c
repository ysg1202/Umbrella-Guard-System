/*
 * iot_server.c - IoT 우산 보관 시스템 멀티스레드 TCP 서버
 *
 * pthread 기반으로 접속 클라이언트마다 스레드(clnt_connection)를 붙여
 * 아두이노 / DB클라이언트 / 블루투스클라이언트를 동시 수용한다.
 * 클라이언트 등록 시 이름 태그(UMB_ARD, UMB_DB, YGY_BLT 등)를 매칭해,
 * 한 클라이언트의 메시지를 다른 특정 클라이언트로 전달하는 라우팅 구조.
 *
 * 컴파일: gcc -o iot_server iot_server.c -lpthread
 * 실행:   ./iot_server <port>
 *         (같은 폴더에 idpasswd.txt 필요: "아이디 비밀번호" 한 줄씩)
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
